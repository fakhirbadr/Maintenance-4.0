...
export default function ConfigActifsDialog({ open, onClose }) {
  ...
  const loadData = async () => {
    try {
      ...
      const merged = actifsData.map((actif) => {
        const persoEntry = personnelsData.find(
          (p) => p.actifId?._id === actif._id
        );
        return {
          ...actif,
  ...
  const handleEdit = (actif) => {
    ...
    setPersonnel({
      ...
      infirmiere1:
        actif.personnel?.find((p) => p.role === "infirmiere1")?.nom || "",
      infirmiere2:
        actif.personnel?.find((p) => p.role === "infirmiere2")?.nom || "",
      technicien:
        actif.personnel?.find((p) => p.role === "technicien")?.nom || "",
    });
  };
        ...
        {selectedActif ? (
          ...
          <Box>
            ...
            <TextField
              ...
              onChange={(e) =>
                setPersonnel({ ...personnel, medecin: e.target.value })
              }
            />
            <TextField
              label="Infirmière 1"
              ...
              onChange={(e) =>
                setPersonnel({ ...personnel, infirmiere1: e.target.value })
              }
            />
            <TextField
              label="Infirmière 2"
              ...
              onChange={(e) =>
                setPersonnel({ ...personnel, infirmiere2: e.target.value })
              }
            />
            <TextField
              label="Technicien"
              ...
              onChange={(e) =>
                setPersonnel({ ...personnel, technicien: e.target.value })
              }
            />
          </Box>
        ) : (
